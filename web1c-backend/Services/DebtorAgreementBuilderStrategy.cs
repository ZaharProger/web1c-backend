...
namespace web1c_backend.Services
{
    public class DebtorAgreementBuilderStrategy : IDataBuilderStrategy
    {
        ...
        {
            IQueryable<En_debtor_agreement> collection;

            if (queryParams.Key.Equals(""))
            {
                collection = from debtorAgreement in context.DebtorAgreements
            ...
            else
            {
                collection = from debtorAgreement in context.DebtorAgreements
                             ...
                             where debtorAgreement.debtor_id.ToString().Equals(queryParams.Key)
