...
namespace web1c_backend.Controllers
{
    ...
    public class EventRecordController : Web1cController
    {
        ...
        [HttpGet]
        public async Task<JsonResult> GetEventRecordHandler([FromQuery] GetParams getEventParams)
        {
            ...
            if (sessionId != null)
            {
                foundData = new DataBuilder()
                    .BuildFromResponse(new EventRecordBuilderStrategy(), getEventParams);
            }
            ...
            var response = new NestedDataResponse<EntityWithRoute>()
            {
                ...
                Data = getEventParams.Type != 4 ?
                    foundData
                    :
                    foundData != null && foundData.Length != 0 && getEventParams.Type == 4 ?
                    foundData
                        .Take(1)
                        .ToArray()
                    :
                    foundData,
                RelatedEvents = Array.Empty<En_event_record>(),
